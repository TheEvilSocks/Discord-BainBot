...
for(i = 0; i < weaponFiles.length; i++){
	var weapon = JSON.parse(fs.readFileSync(weaponFiles + weaponFiles[i], 'utf8'));
...
module.exports = {
	information : {
		moduleName : "betaweapon"
	},
	cooldown : 1000,
	description : "!betaweapon <weapon> - Shows information about a weapon",
	permissions : {
		groups: ["default"]
	},
	action : function (client, e) {
		...
		var found = weapons.filter(function(obj){
			return obj.name.toLowerCase() == e.args[0].toLowerCase() || obj.fullName.toLowerCase() == e.args[0].toLowerCase() || obj.wikiName.toLowerCase() == e.args[0].toLowerCase() || obj.image.toLowerCase() == e.args[0].toLowerCase()
		});
		
		if(found.length == 0)
			found = weapons.filter(function(obj){
				return obj.fullName.toLowerCase().indexOf(e.args[0].toLowerCase()) > -1;
			});
