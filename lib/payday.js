					...
					sockFunc.isShared(steamID, function(err_,isShared){
						...
						sockFunc.getSteamProfile(steamID, function(err__, profile){
							...
							try{
								...
								var primaryGun = weapons[weaponIDs[getStat(payday, "equipped_primary").value-1]] || {fullname: "UNKNOWN", image: "unknown"};
								var secondaryGun = weapons[weaponIDs[getStat(payday, "equipped_secondary").value-1]] || {fullname: "UNKNOWN", image: "unknown"};
								primaryGun.kills = getStat(payday, "weapon_kills_" + weaponIDs[getStat(payday, "equipped_primary").value-1]).value || 0;
								secondaryGun.kills = getStat(payday, "weapon_kills_" + weaponIDs[getStat(payday, "equipped_secondary").value-1]).value || 0;
...
var characterList = {
	...
	"character_used_ecp_female": "Hila",
	"character_used_myh": "Duke",
	"character_used_joy": "Ethan (?)"
}
